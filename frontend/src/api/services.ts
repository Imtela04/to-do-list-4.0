...
export const createTask = (data: TaskPayload): Promise<AxiosResponse<Task>> =>  client.post('/tasks/', data, { _silent: true } as never);
...
export const toggleTask = (id: number, completed: boolean, pinned?: boolean) =>   client.patch(`/tasks/${id}/`, { completed, ...(pinned !== undefined && { pinned }) });

// ── Categories ────────────────────────────────────────────────
export const getCategories  = ():                       Promise<AxiosResponse<Category[]>> => client.get('/categories/');
export const createCategory = (data: CategoryPayload): Promise<AxiosResponse<Category>> =>  client.post('/categories/', data, { _silent: true } as never);
...
export const createStickyNote = (data: NotePayload): Promise<AxiosResponse<StickyNote>> =>  client.post('/sticky-notes/', data, { _silent: true } as never);
