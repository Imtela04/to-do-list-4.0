...
export default function Pomodoro({ onClose }) {
  ...
  const handleSessionComplete = useCallback(async () => {
    ...
    if (isWork) {
      ...
      try {
        ...
        dispatch({ type: 'POMODORO_COMPLETE', payload: res.data });
        setXpToast(`+5 XP`);
  ...
  const linkedTask = state.tasks.find(t => t.id === parseInt(taskId));
  ...
  return (
    <div className={styles.panel}>
      ...
      <div className={styles.taskLink}>
        ...
        {linkedTask && (
          <span className={styles.linkedTask}>
